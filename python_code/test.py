...
from exp_params import seed, K, Nr, fc, BW, alg, aoa_res, T_res, tau,NS,num_of_BSs,input_power
...
def sweep_input_power(model_path,num_users,input_power_list,band,no_NN,alg, BS_num=1,tau =tau,NS=NS):
    ...
    if BS_num == "all":
        BS_list = list(range(1,num_of_BSs+1))
    else:
        BS_list = [BS_num]    
    ...
    for input_power in input_power_list:
        ...
        for BS_num in BS_list:
            ...
            for comb in comb_indices:
                ...
                if too_close(ue_locs_array,70): continue ######

                

                error = test_1sample(model, ue_locs_array, input_power=input_power, toPrint=False, bands=bands,alg=alg, BS_num=BS_num,NS=NS)
                total_error += error
                error_list.append(error)
                valid_comb_count += 1
                if error > 20: problematic_locs.append(ue_locs)
        ...
        print(f"{len(problematic_locs)} examples with error greater then 20[m]:{problematic_locs[:min(len(problematic_locs),20)]}")

        ##############3
        # plt.figure(figsize=(8, 5))
        # plt.hist(error_list, bins=50, color='skyblue', edgecolor='black')
        ...
        # plt.tight_layout()
        # if band == 0:
        #     plt.savefig(f"error_hist_net_BS{BS_num}_{num_users}UEs.png")
        # if band != 0:
        #     plt.savefig(f"error_hist_MUSIC_BS{BS_num}_{num_users}UEs.png")    
    return avg_errors, median_errors        

def sweep_number_of_samples(model_path,num_users,NS_list,band,no_NN,alg, BS_num=1,tau =tau,input_power = input_power):
    #for a given method compute mse vs number of samples 
    # band = 0 for multiband!!!! 
    # band = 1 for single band 6G ,band = 2 for single band 12G ,band = 3 for 18G, band = 4 for 24G
    torch.manual_seed(seed) 
    if band == 0: 
        ...
        for BS_num in BS_list:
            ...
            for comb in comb_indices:
                ...
                for row in comb:
                    ue_info = get_ue_info_by_row(csv_file, row)
                    ue_locs.append(ue_info['ue_loc'][:2])
                    if not(-80<ue_info["aoa"][0]<80): continue
                ue_locs_array = np.array(ue_locs)
                if too_close(ue_locs_array,100): continue
                y, data = get_channel(ue_locs_array, band,BS_num=BS_num, input_power=input_power,NS=NS)
                error = test_1sample(model, ue_locs_array, input_power=input_power, toPrint=False, bands=bands,alg=alg, BS_num=BS_num,NS=NS)
                total_error += error
                error_list.append(error)
                valid_comb_count += 1
...
def test_1sample(model, ues_pos, tau =tau ,toPlot=False, toPrint=True, name=None,zoom = False, input_power=None, bands=None,alg=alg, BS_num=1,NS =NS):
    if not name:
    ...
    for j, band in enumerate(bands):
        ...
        y, data = get_channel(ues_pos, band,BS_num=BS_num, input_power=input_power,NS=NS)
        per_band_y.append(y)
...
if __name__ == "__main__":
    ...
    #     print("-"*40)
    #     print(f"BS {BS}")
    #     avg_errors, median_errors = sweep_input_power(model_path,2,[10],4,no_NN,alg, BS_num=BS)  
    #     print(f"avg = {avg_errors}",f"madian = {median_errors}")
    # no_NN = 1 
    # alg = 'MUSIC'
    # for BS in range(1,num_of_BSs+1):
    #     print("-"*40)
    #     print(f"BS {BS}")
    #     avg_errors, median_errors = sweep_input_power(model_path,2,[10],4,no_NN,alg, BS_num=BS)  
    #     print(f"avg = {avg_errors}",f"madian = {median_errors}")
    ...
    #     print("-"*40)
    #     print(f"BS {BS}")
    #     avg_errors, median_errors = sweep_input_power(model_path,2,[10],0,no_NN,alg, BS_num=BS)  
    #     print(f"avg = {avg_errors}",f"madian = {median_errors}")
