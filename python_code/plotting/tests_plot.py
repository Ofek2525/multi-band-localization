...
def plots_of_NS_MultiBandNet_and_music(results,NS_list,num_users,model_path): 
    # Convert results dict to a readable format
    band_labels = {
        0: 'Multiband (with NN)',
        1: '6GHz (no NN)',
    ...
    for band_idx, (avg_list, median_list) in results.items():
        label = band_labels[band_idx]
        for NS, avg, median in zip(NS_list, avg_list, median_list):
            rows.append({
                'Band': label,
                'NS': NS,
                'Avg Error [m]': avg,
    ...
    # Save CSV
    save_dir = os.path.dirname(model_path)
    csv_path = os.path.join(save_dir, f"error_metrics_vs_NS_{num_users}UEs.csv")
    df.to_csv(csv_path, index=False)

    plt.figure(figsize=(8, 5))
    plt.plot(NS_list, results[4][0], marker='o', color='yellow', label='Avg 24GHz (no NN)')
    plt.plot(NS_list, results[3][0], marker='o', color='blue', label='Avg 18GHz (no NN)')
    plt.plot(NS_list, results[2][0], marker='o', color='green', label='Avg 12GHz (no NN)')
    plt.plot(NS_list, results[1][0], marker='o', color='orange', label='Avg 6GHz (no NN)')
    plt.plot(NS_list, results[0][0], marker='o', color='red', label='Avg multiband (with NN)')
    
    plt.title(f"Error vs NS - {num_users} UEs")
    plt.xlabel("NS")
    plt.ylabel("Error [m]")
    plt.grid(True)
    plt.legend()
    ...
    # Save to same folder as model_path
    save_dir = os.path.dirname(model_path)
    plt.savefig(os.path.join(save_dir, f"Avg_error_vs_NS_{num_users}UEs.png"))
    plt.savefig(os.path.join(ROOT_DIR,"results", f"Avg_error_vs_NS_{num_users}UEs.png"))
    plt.close()
